...
void readLineFromFd( int fd, char * buff) {
    ...
    while(1) {
        ...
        ptr ++;
        // Check to see if pointer is past the buffer
        if( (ptr-buff) > 31){
            // add a new line character as the last character and break from the loop
            * (ptr-1) = '\n';
            break;
        }
...
int main( int argc, char ** argv)
{
    ...
    char * end = NULL;
    globals.port = strtol( argv[1], & end, 10); // Changes arg 1 to long int
    if( * end != 0) die( "bad port %s", argv[1]); // if end is anything but zero die
    strncpy( globals.password, argv[2], sizeof( globals.password)); // copies arg2 to password
    globals.password[ sizeof(globals.password)-1] = 0; // changes last index to password to 0
    strncpy( globals.secret, argv[3], sizeof( globals.secret)); // same as above
